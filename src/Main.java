...
public class Main {
    ...
    public static void main(final String[] args) {
        ...
        Map<String, Long> result = boys.stream().filter(b -> b.getAge() > 18).map(Boy::getName)
                .distinct().sorted().limit(4)
                .collect(Collectors.toMap(b -> b, p -> boys.stream()
                        .filter(b -> b.getName().equals(p)).count() - 1));
